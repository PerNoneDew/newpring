...
import { User, Booking, Room, DashboardMetrics, EventBooking, Service, PaymentConfig, PaymentMethod, StaffAccount, BusinessInfo, Cottage } from './types';
import type {
  DatabaseRoom,
  DatabaseCottage,
  DatabaseService,
...
export interface BookingContextType {
  ...
  addCottage: (cottage: Cottage) => void;
  updateCottage: (id: string, cottage: Partial<Cottage>) => void;
  deleteCottage: (id: string) => void;
  addService: (service: Service) => void;
  updateService: (id: string, service: Partial<Service>) => void;
  deleteService: (id: string) => void;
  setEventTypePrice: (type: string, price: number) => void;
  addEventType: (type: string, name: string, description: string, price: number) => void;
  updateEventType: (type: string, name: string, description: string, price: number) => void;
...
export function BookingProvider({ children }: { children: ReactNode }) {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        if (cottagesData.data) {
          setCottages(cottagesData.data.map((cottage) => dbCottageToCottage(cottage as DatabaseCottage)));
        }
  ...
  // Cottages
  const addCottage = async (cottage: Cottage) => {
    try {
      const { data, error } = await supabase.from('cottages').insert({
        cottage_number: cottage.cottageNumber,
        name: cottage.name,
        description: cottage.description || null,
        price_per_night: cottage.pricePerNight,
        capacity: cottage.capacity,
        status: cottage.status,
        image_url: cottage.image || null,
      }).select().maybeSingle();
      if (error) throw error;
      if (data) setCottages(prev => [...prev, dbCottageToCottage(data as DatabaseCottage)]);
    } catch (error) {
      console.error('Error adding cottage:', error);
    }
  };

  const updateCottage = async (id: string, updates: Partial<Cottage>) => {
    try {
      const dbUpdates: Record<string, unknown> = {};
      if (updates.cottageNumber) dbUpdates.cottage_number = updates.cottageNumber;
      ...
      const { error } = await supabase.from('cottages').update(dbUpdates).eq('id', id);
      if (error) throw error;
      setCottages(prev => prev.map(c => c.id === id ? { ...c, ...updates } : c));
    } catch (error) {
  ...
  const handleAddEventType = async (type: string, name: string, description: string, price: number) => {
    setEventTypeLoading(true);
    try {
      const slug = type.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '');
      const { error } = await supabase
        .from('event_type_prices')
        .insert({ event_type: slug, name, description, price });

      if (error) throw error;

      setEventTypePrices(prev => [...prev, { type: slug, name, description, price }]);
    } catch (error) {
  ...
  const handleUpdateEventType = async (type: string, name: string, description: string, price: number) => {
    try {
      const { error } = await supabase
        .from('event_type_prices')
      ...
      setEventTypePrices(prev => prev.map(p => p.type === type ? { ...p, name, description, price } : p));
    } catch (error) {
